...
func encodeLoginPushBmsg(lp LoginPush) []byte {
	...
	st := NewEncoder(&stBuf)
	if lp.Settings.MusicOn { st.WriteFieldHeader(1, 0); st.WriteVarint(1) }
	if lp.Settings.SfxOn { st.WriteFieldHeader(2, 0); st.WriteVarint(1) }
	st.WriteFieldHeader(3, 0); st.WriteVarint(uint64(lp.Settings.Language))
	st.WriteFieldHeader(4, 0); st.WriteVarint(uint64(lp.Settings.Quality))
	if lp.Settings.AutoBattle { st.WriteFieldHeader(5, 0); st.WriteVarint(1) }
